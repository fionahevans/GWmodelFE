...
gwr.mixed.trace.fast <- function(x1, x2, y, adaptive=F, bw=sqrt(var(loc[,1])+var(loc[,2])),
...
gwr.q.fast <- function(x, y, adaptive=F, bw=sqrt(var(loc[,1])+var(loc[,2])),
                  kernel, dMat)
{
  if (missing(dMat)) {
    cat("Error: distance matrix missing")
    exit()
  }
